module controlador_JER (
   ...
   input  wire reset,
   input  wire [15:0] entrada_dados,
	input  wire sinal_prox_pixel,
   output wire hsync,
   output wire vsync,
   output wire [7:0] red,
   ...
   assign modo_janela = entrada_dados[15];
   ...
   // AUMENTADO PARA 7 BITS PARA INCLUIR O MODO_JANELA
   reg [6:0] config_anterior; 
   wire config_mudou;
   
   // Agora monitoramos também o bit modo_janela
   wire [6:0] config_atual = {modo_janela, entrada_algoritmo, entrada_zoom};
   
   assign config_mudou = (config_atual != config_anterior);
   
   always @(posedge clock_25MHz) begin
      if (reset) begin
         config_anterior <= 7'b0000000;
   ...
   assign altura_destino = (altura_imagem > VGA_HEIGHT) ? VGA_HEIGHT : altura_imagem;
	...
	// ========= DETECÇÃO DE BORDA DO SINAL NEXT_PIXEL =========
   reg next_pixel_prev;
   wire next_pixel_edge;

   always @(posedge clock_25MHz) begin
      if (reset)
         next_pixel_prev <= 1'b0;
      else
         next_pixel_prev <= sinal_prox_pixel;
   end

   // Detecta borda de subida (0 -> 1) vinda do HPS
   assign next_pixel_edge = sinal_prox_pixel && !next_pixel_prev;
   ...
   assign enable_contador = modo_janela ? next_pixel_edge : (usar_media_blocos ? pixel_pronto : 1'b1);
   ...
   always @(posedge clock_25MHz) begin
      if (reset || config_mudou) begin
         ...
         modo_vga <= modo_janela ? 0 : 1;  // Se modo janela, não entra em modo VGA
         processamento_completo <= 1;
      end
   end
   
   // Sinal de status: pronto quando está em modo VGA e processamento completo
   // ou quando modo janela e processamento completo
   assign pronto = modo_janela ? 1'b1 : (modo_vga && processamento_completo);
   ...
   // ========= Saída pixel_retorno para modo janela =========
   reg pixel_retorno_valido;
   
   // Lógica auxiliar para pegar as coordenadas que estão sendo usadas na leitura da RAM
   // O x_ram_in_select e y_ram_in_select já contêm a coordenada da imagem ORIGINAL calculada
   
   always @(posedge clock_25MHz) begin
      if (reset || config_mudou) begin
         pixel_retorno <= 19'b0;
         pixel_retorno_valido <= 1'b0;
      end else if (modo_janela && !modo_vga && !controle_ram) begin
         if ((y_proc < altura_destino) && (x_proc < largura_destino)) begin
            
            // Lógica simplificada: Se o pixel final seria válido, enviamos a COORDENADA.
            // Nota: Em algoritmos de média, isso enviará a última coordenada usada ou 
            // você pode optar por enviar a coordenada "central" do bloco. 
            // Para NNI e Replicação, x_ram_in_select é exato.
            
            if (usar_media_4x) begin
               if (estado_media == 5'd16) begin
                  // Concatena Y (bits 14:8) e X (bits 7:0). Bits superiores 0.
                  pixel_retorno <= {4'b0, y_ram_in_select[6:0], x_ram_in_select[7:0]};
                  pixel_retorno_valido <= 1'b1;
               end else begin
                  pixel_retorno_valido <= 1'b0;
               end
            end else if (usar_media_2x) begin
               ...
               // Algoritmos NNI e Replicação (sem estados de espera de média)
               pixel_retorno <= {4'b0, y_ram_in_select[6:0], x_ram_in_select[7:0]};
               pixel_retorno_valido <= 1'b1;
            end
         end else begin
   ...
   // ========= Escrita na RAM (desabilitada em modo janela) =========
   always @(posedge clock_25MHz) begin
      if (reset || config_mudou) begin
         endereco_RAM_wr <= 0;
         pixel_RAM_wr    <= 0;
         wren            <= 0;
      end else if (!modo_vga && !controle_ram && !modo_janela) begin  // Não escreve em modo janela
         if ((y_proc < altura_destino) && (x_proc < largura_destino)) begin
            if (usar_media_4x) begin
               if (estado_media == 5'd16) begin
